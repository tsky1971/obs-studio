...
typedef struct _SYSTEM_THREAD_INFORMATION3 {
	FILETIME KernelTime;
	FILETIME UserTime;
	FILETIME CreateTime;
	DWORD WaitTime;
...
static bool thread_is_suspended(DWORD process_id, DWORD thread_id)
{
	...
	SYSTEM_THREAD_INFORMATION3 *sti;
	SYSTEM_THREAD_INFORMATION3 *info = NULL;
	sti = (SYSTEM_THREAD_INFORMATION3*)((BYTE*)spi + sizeof(*spi));
