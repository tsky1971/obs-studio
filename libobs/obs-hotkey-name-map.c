...
// @tsky rename due to name conflict with real sockets
static void connect_hotkey(obs_hotkey_name_map_edge_t *e,
		...
		const char *key, size_t l, int v)
{
	if (node->is_leaf && l > 0) {
		obs_hotkey_name_map_node_t *new_node_ = new_node();
		connect_hotkey(edge, new_node_);

		obs_hotkey_name_map_edge_t *edge =
			da_push_back_new(new_node_->children);
		connect_hotkey(edge, node);
